...
exports.main = async (event, context) => {
  try {
   ...
   let newOrder = order.list.reduce((prev, item) => {
    ...
    // 2, 定义存储数据的
    let summaryList = []
    let summaryRow = ['性别', '规格', '产品名称', '总计']
    summaryList.push(summaryRow)
    ...
    let detailedList = []
    let detailedRow = ['id', '学校', '年级', '班级', '姓名', '性别', '下单时间', '商品名称', '购买数量', '规格', '备注' ] //表属性
    ...
    var buffer = xlsx.build([
      { name: "证订明细表", data: detailedList, options: sheetOptions }
    ])
